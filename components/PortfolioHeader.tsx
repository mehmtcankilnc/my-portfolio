    ...
    <header className="sticky top-0 z-100 h-20 bg-white border-b-4 border-black px-4 md:px-6 flex justify-between items-center">
      <div className="flex items-center gap-3">
        <div className="bg-black text-white p-1.5 md:p-2 border-2 border-black shadow-[3px_3px_0px_var(--color-industrial-yellow)]">
          <SmoothIcon name="terminal-outlined" color="#FFF" size={24} />
        </div>
        <Link
          href="/"
      ...
      >
        {isMenuOpen ? (
          ...
          <SmoothIcon name="menu-right" color="#FFF" size={28} />
        )}
      </button>
      <nav className="hidden md:flex gap-4 lg:gap-8 font-jetbrains text-sm font-bold uppercase items-center">
        <a
          href="https://mehmtcankilinc.com/smooth-icon"
          target="_blank"
          rel="noopener noreferrer"
          className="mr-2 px-3 py-1.5 bg-[#EF7C00] text-white border-2 border-black shadow-[3px_3px_0px_black] hover:shadow-[5px_5px_0px_black] hover:-translate-y-0.5 -rotate-2 hover:rotate-0 transition-all flex items-center gap-1.5"
          title="Smooth Icon Library'yi Keşfet"
        >
          <SmoothIcon name="sparkles" color="#FFF" size={16} />
          SMOOTH ICON
        </a>
        {navLinks.map((link) => {
          ...
          return (
            <div key={link.id} className="relative group">
              ...
              >
                {link.label}
                {link.hasDropdown && (
                  <SmoothIcon name="chevron-down" color="#000" size={14} />
                )}
              </Link>
              {link.hasDropdown && (
      ...
      >
        <a
          ...
          rel="noopener noreferrer"
          onClick={() => setIsMenuOpen(false)}
          className="flex items-center justify-between px-6 py-6 bg-[#EF7C00] text-white border-b-4 border-black active:bg-[#d66e00] transition-colors"
        >
          <div className="flex items-center gap-2">
            <SmoothIcon name="sparkles" color="#FFF" size={24} />
            <span className="text-xl">SMOOTH ICON</span>
          </div>
          <SmoothIcon name="arrow-up-right" color="#FFF" size={24} />
        </a>
        {navLinks.map((link) => {
          const isActive = activeSection === link.id;
          return (
              ...
              <div className="flex justify-between items-center w-full">
                ...
                {link.hasDropdown && (
                  ...
                  >
                    ...
                    >
                      <SmoothIcon name="chevron-down" color="#000" size={24} />
                    </div>
                  </button>
                )}
              </div>
